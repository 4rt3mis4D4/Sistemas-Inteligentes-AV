...
# -- Separar os dados em treino e teste antes do balanceamento
# -- O conjunto de teste é mantido com a distribuição original (sem SMOTE)
atributos_train, atributos_teste, classe_train, classe_teste = train_test_split(
    dados_atributos_norm,
    dados_classe,
    test_size=0.2,
    random_state=42,
...
# 4. BALANCEAMENTO
# -- Calcula k_neighbors seguro: deve ser menor que o nº de amostras da menor classe
menor_classe = min(Counter(classe_train).values())
k_neighbors_seguro = min(4, menor_classe - 1)
# -- Construção do balanceador 
resampler = SMOTE(random_state=42, k_neighbors=k_neighbors_seguro)
atributos_b, classes_b = resampler.fit_resample(atributos_train, classe_train)
...
gb_grid = {
    'max_iter': [int(x) for x in np.linspace(start=50, stop=200, num=5)],
...
gb_best = HistGradientBoostingClassifier(**gb_hyperparameters.best_params_, random_state=42)
